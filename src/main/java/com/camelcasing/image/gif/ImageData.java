...
public class ImageData {
	...
	private void getGIFOptions(){
		...
		if(gifOptions.getImageSize() != null){
			...
			if(newSize[0] < width) width = newSize[0];
			if(newSize[1] < height) height = newSize[1]; 
		}
		
		if(gifOptions.getMemeText() != null) addMeme = true;
	...
	private int getColourTableSize(){
		if(colourCount >= 16){
			...
			return(GIFUtils.getBitsRequiredFromDecimal(colourCount));
		}
	}
	...
	public void addColourTableBytes(){
		...
		int extraColour = (int)Math.pow(2, bitSize + 1);
		...
		//bitSize = GIFUtils.numberOfBitsRequired(colourTable.getColourTableSize());
		bitSize = GIFUtils.numberOfBitsRequired(7);
	}
